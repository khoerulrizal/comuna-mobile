// Pengajuan Lembur Terkirim — hero sukses + ringkasan + estimasi. Ikut desain.
...
export default function LemburSuksesScreen() {
  const insets = useSafeAreaInsets();
  const p = useLocalSearchParams<{ id: string; date: string; time: string; hours: string; reason: string; comp: string }>();
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.neutral[25], paddingTop: insets.top }}>
      ...
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, paddingVertical: 10, backgroundColor: "#fff", borderBottomWidth: 1, borderBottomColor: colors.neutral[100] }}>
        <View style={{ width: 50 }} />
        <Txt size={14} weight="extrabold" color={colors.neutral[900]}>Pengajuan Lembur</Txt>
        <Pressable onPress={() => router.replace("/lembur")} hitSlop={8}><Txt size={13} weight="bold" color={colors.brand[600]}>Selesai</Txt></Pressable>
      </View>

      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 100 }}>
        <LinearGradient colors={[colors.brand[600], colors.coral[500]]} start={{ x: 0, y: 0 }} end={{ x: 0.9, y: 1 }} style={{ borderRadius: 24, paddingVertical: 26, paddingHorizontal: 20, alignItems: "center" }}>
          ...
          <Txt size={12.5} color="rgba(255,255,255,0.92)" style={{ textAlign: "center", marginTop: 4 }}>Permintaan lemburmu diteruskan ke penyetuju untuk di-review</Txt>
        ...
        <Card pad={0} radius={18}>
          <SRow label="Tanggal" value={p.date ?? "-"} icon="calendar" color={colors.brand[600]} bg={colors.brand[100]} />
          <Div />
          <SRow label="Jam Lembur" value={p.time ?? "-"} icon="clock" color={colors.mint[700]} bg={colors.mint[100]} />
          <Div />
          <SRow label="Durasi" value={p.hours ?? "-"} icon="check" color={colors.coral[700]} bg={colors.coral[100]} />
          {p.reason ? <><Div /><SRow label="Alasan" value={p.reason} icon="star" color={colors.amber[700]} bg={colors.amber[100]} /></> : null}
        </Card>

        {p.comp ? (
          <View style={{ marginTop: 14, padding: 14, borderRadius: 14, backgroundColor: colors.mint[100], flexDirection: "row", alignItems: "center", gap: 12 }}>
            <View style={{ width: 40, height: 40, borderRadius: 11, backgroundColor: "#fff", alignItems: "center", justifyContent: "center" }}><Icon name="wallet" size={18} color={colors.mint[700]} strokeWidth={2.2} /></View>
            <View style={{ flex: 1 }}>
              <Txt size={12} weight="bold" color={colors.mint[700]}>Estimasi kompensasi</Txt>
              <Txt size={17} weight="extrabold" color={colors.mint[700]} style={{ marginTop: 2 }}>{p.comp}</Txt>
              <Txt size={10.5} color={colors.mint[700]} style={{ marginTop: 1, opacity: 0.8 }}>Final dikonfirmasi saat payroll run</Txt>
            </View>
          </View>
        ) : null}
      </ScrollView>
      ...
      <View style={{ position: "absolute", left: 0, right: 0, bottom: 0, flexDirection: "row", gap: 10, paddingHorizontal: 16, paddingTop: 12, paddingBottom: insets.bottom + 12, backgroundColor: "#fff", borderTopWidth: 1, borderTopColor: colors.neutral[100] }}>
        <Pressable onPress={() => router.replace("/lembur")} style={{ flex: 1, paddingVertical: 14, borderRadius: 16, borderWidth: 1, borderColor: colors.neutral[200], alignItems: "center" }}>
          <Txt size={14} weight="bold" color={colors.neutral[700]}>Kembali</Txt>
        </Pressable>
        <Pressable onPress={() => router.replace(`/lembur/${p.id}`)} style={{ flex: 1 }}>
